...
class InnerPage extends PureComponent<any, any> {
  ...
  async getComponent(pathName:string){
    ...
    if(pathName===appendPath('/')||pathName==='/'){
      ...
      if(!systemRootPath||systemRootPath===appendPath('/')){
        // 设置当前的根目录组件
        this.setState({Component:Home});
        return;
      }
      //自定义域名页面 嵌入iframe
      if(/http[s]{0,1}:\/\/([\w.]+\/?)\S*/.test(systemRootPath)){
        this.setState({
  ...
  componentDidMount(){
    ...
    ThemeManage.loadSystemLayout();
  }
  
  render() {
    let {systemLayout={},location,userMenus=[]} = this.props;
    ...
    return  (
        <LayoutFramework menu={menus} currPath={appRoute.path} smallLogo={systemLayout.small_logo} logo={systemLayout.logo} layoutType={systemLayout.frame}>
          <DynamicLoading isComplate={!loadingPage}>
            {Component?<Component route={appRoute}/>:<div style={{width:'100%',height:'100%'}}></div>}
          </DynamicLoading>
        </LayoutFramework>
    )
  }
